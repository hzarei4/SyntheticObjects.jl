...
```
"""
function filaments!(obj; intensity = 1.0, radius = 0.8, rand_offset=0.05, rel_theta=1.0, num_filaments=50, apply_seed=true, thickness=0.8)
...
#filaments!(obj::Array{T, 2}; kwargs...) where {T}= filaments!(Tuple((sz..., 1)); kwargs...)[:, :, 1]
...
filaments(sz; intensity=one(Float32), radius = 0.8, rand_offset=0.2, num_filaments=50, rel_theta=1.0, apply_seed=true, thickness=0.8) = filaments(Float32, sz; intensity=intensity, radius=radius, rand_offset=rand_offset, num_filaments=num_filaments, apply_seed=apply_seed, thickness, rel_theta=rel_theta) 
filaments(sz::Tuple{Int, Int}; kwargs...) = filaments(Tuple((sz..., 1)); kwargs...)[:, :, 1]
